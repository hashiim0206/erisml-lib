...
\title{\textbf{Geometric Ethics: A Computational Foundation for\\Objective Morality via Topological Optimization}}
...
\begin{abstract}
For millennia, the metaethical debate has been trapped in a false dichotomy: either objective moral values are grounded in a transcendent, metaphysically necessary realm (Platonism/Theism), or they are entirely subjective artifacts of cultural evolution (Relativism). This paper introduces a rigorous third paradigm---\emph{Computational Objectivity}. By mapping multi-agent social interactions onto a high-dimensional Whitney stratified space, we demonstrate that moral reasoning is mathematically equivalent to $A^*$ pathfinding on a constrained topological manifold. Under this framework, moral obligations are not divine commands or cultural preferences; they are pre-compiled heuristic vectors $h(n)$ that steer autonomous agents toward a $D_4$ gauge symmetry, minimizing behavioral friction and preventing catastrophic systemic collapse. Five principal results support the framework: (1) the \emph{Structured Preservation Theorem}, proving that any non-trivial, continuous moral evaluation must be equivalent to one defined on the moral manifold; (2) the \emph{Moral Noether Theorem}, deriving harm conservation from re-description invariance; (3) the \emph{$D_4$ Gauge Group Theorem}, establishing the symmetry group of deontic structure; (4) the \emph{Information Monotonicity Theorem}, proving that scalar moral evaluations are irreversibly lossy; and (5) the \emph{No Escape Theorem}, demonstrating that sufficiently capable AI systems are necessarily subject to geometric moral constraints. Empirical validation against 20,030 advice-column letters and 109,294 cross-lingual passages confirms that the geometric structure is not an artifact of the formalism but a stable feature of moral cognition across cultures, languages, and millennia. This approach redefines ethics as a computable, objective branch of engineering---proving that absolute moral frameworks are mathematically emergent properties of constrained geometries. The reference implementation, ErisML, is open-source and available on PyPI.
\end{abstract}
...
%==============================================================================
\section{Introduction: The False Dichotomy of Metaethics}
\label{sec:introduction}
%==============================================================================

\subsection{The Category Error at the Foundation}

The historical pursuit of ``Objective Morality'' has been consistently hindered by a foundational category error. Classical apologists and moral realists assert that an objective moral law requires a Lawgiver, relying on logical frameworks that illegally extend human causal heuristics beyond their domain of calibration. The argument from moral realism to divine necessity follows a well-documented trajectory: domain-calibrated intuitions are codified into formal tools (modal logic, the principle of sufficient reason); the tools' internal structure is then treated as a transparent mirror of reality; conclusions are ``read off'' the formalism as though they were discoveries about the world rather than consequences of modeling choices \cite{bond2026pragmatist}. This is the common failure mode of reification---and it afflicts theological metaethics at its root.

Conversely, secular philosophers, correctly identifying the logical flaws in these metaphysical arguments, often retreat into moral relativism, concluding that morality is merely a localized, subjective cultural construct. If there is no Lawgiver, the reasoning goes, there can be no Law. But this conclusion follows only if one accepts the theistic framing's hidden premise: that objectivity requires transcendent grounding.

Both frameworks fail because they misunderstand the nature of objectivity.

\subsection{Objectivity Without Transcendence}

\emph{Objectivity does not require a transcendent origin; it requires mathematical inevitability under a defined set of constraints.}

Just as a water droplet assumes a spherical geometry not by divine decree but because the sphere is the global minimum of the surface-tension energy functional, multi-agent networks converge on specific behavioral geometries to minimize social and physical friction. The sphere is not ``subjective'' simply because no deity shaped it. Its geometry is an objective consequence of the physics of surface tension. The objectivity is \emph{computational}---it emerges from optimization under constraint.

This paper formalizes \textbf{Geometric Ethics}, proposing that objective morality is a fundamentally computable phenomenon---an exercise in topological optimization on the space of multi-agent configurations. The framework:

\begin{enumerate}[label=(\roman*)]
    \item Models the space of ethically relevant configurations as a \emph{Whitney stratified space}---a union of smooth manifolds of varying dimensions connected along boundary strata that encode moral discontinuities.
    \item Defines \emph{behavioral friction} as the cost function on this space, measuring the energy a multi-agent system must expend to process the consequences of an action.
    \item Demonstrates that moral reasoning is equivalent to $A^*$ pathfinding on this manifold, where moral obligations serve as pre-compiled heuristic functions $h(n)$.
    \item Proves that the optimal equilibrium configuration exhibits $D_4 \times U(1)_\harm$ gauge symmetry---the symmetry group of Hohfeldian deontic structure.
    \item Derives a conservation law for harm from the re-description invariance of the moral Lagrangian, via Noether's theorem.
\end{enumerate}

\subsection{The Kuhnian Context}

Thomas Kuhn's \emph{The Structure of Scientific Revolutions} \cite{kuhn1962} observed that paradigm shifts in science occur not because the old paradigm is definitively refuted, but because a new paradigm solves problems the old one could not while preserving its successes. The shift from Ptolemaic to Copernican astronomy, from Newtonian to Einsteinian mechanics, from classical to quantum physics---each replaced a framework that had become increasingly burdened with epicycles and ad hoc patches.

Metaethics is overdue for such a shift. The theological paradigm has accumulated millennia of epicycles: theodicies to explain why an omnipotent moral Lawgiver permits suffering, divine-command theories to explain why moral obligations track God's will rather than vice versa, and increasingly baroque modal-logic arguments to establish the metaphysical necessity of moral truth. The relativist paradigm, for its part, cannot account for the striking cross-cultural convergence of basic moral norms---a convergence confirmed empirically by our analysis of 109,294 passages spanning 3,000 years and 11 languages (\S\ref{sec:evidence}).

Geometric Ethics resolves both: moral convergence is explained not by a transcendent Lawgiver but by the mathematics of constrained optimization. Moral variation across cultures is explained not by subjectivity but by different positions on the same manifold---different local optima of the same global landscape. And moral progress is explained as movement toward global rather than merely local minima of behavioral friction.
...
Section~\ref{sec:manifold} constructs the moral manifold and defines behavioral friction. Section~\ref{sec:pathfinding} develops the $A^*$ pathfinding interpretation and the role of obligation vectors as heuristics. Section~\ref{sec:gauge} establishes the $D_4$ gauge symmetry, derives the conservation of harm via Noether's theorem, and proves the key structural theorems. Section~\ref{sec:evidence} presents the empirical evidence. Section~\ref{sec:engineering} describes the engineering methodology (Philosophy Engineering) and the reference implementation (ErisML). Section~\ref{sec:conclusion} addresses the paradigm shift and its implications.
...
In human cognitive architecture, computing the full tensor contraction of the entire social manifold for every decision is computationally intractable. The moral manifold is nine-dimensional, the number of stakeholders is large, the temporal horizon extends indefinitely, and the stochastic dynamics are chaotic. Exact computation of the optimal geodesic is NP-hard in general (\cite{bond2026sge}, Theorem 5.3).
...
\begin{proof}[Proof sketch]
Consider the prohibition of murder: $h_{\text{murder}}(n) = \infty$ for states $n$ that involve killing. The true cost $g^*(n, s_g)$ of the cheapest path from a post-murder state to equilibrium includes, at minimum: (i) grief processing across all connected agents; (ii) institutional response (investigation, adjudication, incarceration); (iii) reputational damage propagation through the social network; (iv) deterrence expenditure to prevent recurrence. Empirically, these costs are unbounded in the limit of network connectivity---a single murder in a tightly connected community can destabilize the entire social graph. Hence $h_{\text{murder}}(n) \leq g^*(n, s_g)$, and the heuristic is admissible. Similar arguments apply to theft (property-rights restoration cost) and deception (trust-rebuilding cost). See \cite{bond2026geometric}, Chapter 13 for detailed analysis.
\end{proof}
...
The key insight comes from the structure of \emph{Hohfeldian jural relations}---the fundamental building blocks of legal and moral reasoning identified by Wesley Hohfeld in 1913. Hohfeld showed that all legal relations reduce to four atomic types---\emph{right}, \emph{duty}, \emph{liberty}, and \emph{no-right}---connected by two relations: \emph{correlatives} (right $\leftrightarrow$ duty, liberty $\leftrightarrow$ no-right) and \emph{opposites} (right $\leftrightarrow$ no-right, duty $\leftrightarrow$ liberty).
...
\begin{itemize}
    ...
    \item The rules governing this symmetry are therefore \emph{universally binding and mathematically objective}. A culture that attempts to build a moral framework outside of this symmetric equilibrium will experience rapid, mathematically guaranteed structural collapse---the behavioral friction accumulates until the social fabric tears along stratum boundaries.
\end{itemize}
...
This is exactly what the non-abelian structure of $D_4$ predicts: the generators of a non-abelian group do not commute. If the gauge group were abelian (like $U(1)$ in electromagnetism), moral frameworks would commute, and the order of application would not matter. The empirical non-commutativity is direct evidence for the $D_4$ structure.
...
\begin{itemize}
    ...
    \item \textbf{DEME Architecture:} The Distributed Ethics Module Engine---a governance pipeline where ethics evaluation occurs \emph{before} action execution, in the decision loop, not as a post-hoc filter.
...
%==============================================================================
\section{Conclusion: The Era of Philosophy Engineering}
\label{sec:conclusion}
%==============================================================================

\subsection{The Paradigm Shift}

By stripping ethics of its metaphysical baggage and translating it into the language of differential geometry and computer science, Geometric Ethics provides a unified, computable framework for moral reasoning.

Morality is not a divine command. It is not a cultural preference. It is a \emph{geometric inevitability}---the unique equilibrium configuration that minimizes behavioral friction in a multi-agent system operating under the constraints of bounded rationality, shared environment, and the requirement of representational invariance.

The paradigm shift can be summarized in a table:

\begin{center}
\begin{tabular}{p{3.5cm}p{3.5cm}p{3.5cm}}
\toprule
\textbf{Theological Paradigm} & \textbf{Relativist Paradigm} & \textbf{Geometric Paradigm} \\
\midrule
Moral truth is grounded in a transcendent Lawgiver & Moral truth is a cultural construction & Moral truth is a computational invariant \\
\addlinespace
Objectivity requires metaphysical necessity & Objectivity is impossible & Objectivity requires mathematical inevitability under constraints \\
\addlinespace
Moral convergence explained by shared divine nature & Moral convergence unexplained (or denied) & Moral convergence explained by optimization on a shared manifold \\
\addlinespace
Moral variation is error & Moral variation is fundamental & Moral variation is different positions on the same manifold \\
\addlinespace
Moral progress = approach to God's will & Moral progress is incoherent & Moral progress = movement toward global (vs.\ local) minima of friction \\
\addlinespace
Problem: theodicy & Problem: cannot explain convergence & Problem: specifying the metric \\
\bottomrule
\end{tabular}
\end{center}

\subsection{The Honest Problem}

The Geometric Ethics paradigm is not without its own challenge, and intellectual honesty demands that we name it: \emph{specifying the moral metric $g_{\mu\nu}$}.

The metric encodes the trade-off structure---how much of one moral dimension is worth how much of another. The framework proves that such a metric must exist (Theorem~\ref{thm:structured}) and constrains its symmetries (Theorem~\ref{thm:d4}), but it does not \emph{derive} the metric from first principles. The empirical weights of \S\ref{sec:evidence} provide a ground-state calibration, but the metric is context-dependent (it varies across the manifold), and calibrating it in every context is an ongoing empirical program.
...
\emph{The tensor is not yet fully contracted. The uncertainty has shape. The stratum boundary is near. The holonomy is unknown. The harm is conserved. The residue will remain.}

\emph{Hold the projection. Watch the structure unfold.}
...
A.~H. Bond, \emph{Geometric Ethics: The Mathematical Structure of Moral Reasoning}, v1.11. Preprint, San Jos\'{e} State University, 2026.
