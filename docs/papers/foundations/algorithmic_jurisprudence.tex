...
This is not a deficiency that better legal writing could fix.  It is a \emph{structural property} of natural language: natural language is representation-dependent.  The same legal situation, described differently, can trigger different legal conclusions.  This is precisely the failure mode that the Epistemic Invariance Principle (EIP) was designed to detect and prevent \cite{bond2025eip}.

\subsection{The Proposal: Law as Geometry}

We propose that legal reasoning has \emph{geometric structure} that natural-language practice obscures but cannot eliminate---just as moral reasoning has geometric structure that scalar alignment discards \cite{bond2026geometric}.  Specifically:
...
The LIP is the Epistemic Invariance Principle (EIP) of \cite{bond2025eip} specialized to the legal domain.  It formalizes what the law already demands in principle---equal protection, blindfolded justice---but rarely achieves in practice.
...
The JBIP is the Bond Invariance Principle (BIP) of Geometric Ethics \cite{bond2026geometric} specialized to law.  Where the LIP requires invariance under \emph{all} legally irrelevant transformations, the JBIP specifies the \emph{structural content}: the ``bonds'' that must be preserved are the Hohfeldian legal relations.  The JBIP is stronger than the LIP: it identifies the specific mathematical structure whose preservation defines legal equivalence.
...
In Geometric Ethics, the gauge group $D_4$ was derived from the first four Hohfeldian positions: right, duty, liberty, no-right \cite{bond2026geometric}.  Law requires the \emph{full} Hohfeldian octad:
...
Each Hohfeldian square admits $D_4$ symmetry by the same argument as Theorem 11.4 of \cite{bond2026geometric}: the four positions form two correlative pairs connected by jural opposition (negation) and correlative swap (agent-swap), generating $D_4$.
...
In physics, Noether's theorem \cite{noether1918} derives conservation laws from continuous symmetries of a Lagrangian.  The Moral Noether Theorem \cite{bond2026noether} adapts this to ethics.  In the legal domain, however, a direct application of the continuous Noether theorem faces a structural obstacle: the judicial complex is discrete, and the ``symmetries'' of the legal system (equal protection, due process) are not continuous symmetries of a differentiable action functional---they are \emph{constraints on a graph}.
...
The following empirical results, reported in \cite{bond2026geometric} and summarized here, provide the evidential base:
...
This architecture directly replicates the methodology validated in Geometric Ethics, where nine moral-dimension probes achieved $F_1 = 0.74$--$0.91$ \cite{bond2026geometric}, independently confirmed at $F_1 = 0.74$--$0.91$ by Thiele \cite{thiele2026}.  The legal probes use the same architecture with legal dimension labels.
...
\subsection{Relationship to ErisML}

The existing ErisML library \cite{bond2026erisml} provides the core infrastructure:
...
\begin{itemize}
    ...
    \item \textbf{DEME governance} adapts to \textbf{judicial governance}---a pipeline where legal evaluations occur before action in legal-AI systems.
\end{itemize}

An extension module, tentatively \texttt{erisml.law}, would add statute parsing, precedent-graph construction, and the topological constitutionality test.
...
\begin{enumerate}
    \item \textbf{Calibration of the judicial metric.}  The metric must be calibrated from statutory databases and case-law corpora.  This is a large-scale empirical programme analogous to calibrating the moral metric in Geometric Ethics.

    \item \textbf{Cross-jurisdictional invariance.}  The framework should exhibit invariance across legal systems (common law, civil law, Islamic law, customary law) at the level of structural features, just as the moral framework exhibits cross-lingual invariance.  Empirical testing is needed.
...
\bibitem{bond2025eip}
A.~H. Bond, ``Philosophy Engineering: A Foundation Document for the Discipline,'' v1.0. Technical Report, San Jos\'{e} State University, 2025.
