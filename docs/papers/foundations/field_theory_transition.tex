...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\textit{From Pathfinding to Field Theory}}
...
\title{%
\vspace{-1cm}
{\LARGE\textbf{From Pathfinding to Field Theory}}\\[0.8em]
{\Large The Transition from $A^*$ on the Moral Manifold\\to the Moral Field Equation}\\[1em]
...
\begin{abstract}
The Geometric Ethics framework models moral reasoning as $A^*$ pathfinding on a nine-dimensional Riemannian manifold $\calM$, where obligations define heuristics, the metric tensor encodes trade-offs, and behavioral friction provides the cost function. This paper articulates the transition from pathfinding---which is analogous to classical particle mechanics---to a full moral field theory, analogous to the transition from Newtonian mechanics to general relativity. We identify four concrete mathematical upgrades required: (1)~from scalar heuristics to the dynamic moral metric tensor $g_{\mu\nu}$, (2)~from search-cost minimization to the moral Lagrangian and the Hamilton principle for ethics, (3)~from flat-space pathfinding to the Riemann curvature tensor and moral holonomy, and (4)~from local gauge constraints to a candidate moral field equation relating curvature to moral sources. We report that CHSH Bell tests ($N = 600$) have empirically falsified the original $SU(2)_I \times U(1)_H$ gauge group proposal, establishing the classical $D_4 \times U(1)_H$ as the correct symmetry of the moral manifold. The paper locates each upgrade within the existing framework, identifies what is proven, what is empirically confirmed, and what remains open, and argues that the $A^*$ model and the field theory are not competitors but layers of the same architecture: $A^*$ is the computationally tractable approximation; the field equation is the generating theory from which the approximation derives.
\end{abstract}
...
The central computational claim of Philosophy Engineering is that moral reasoning is equivalent to $A^*$ pathfinding on the moral manifold $\calM$ \cite{bond2026pe}. This claim is precise: optimal moral reasoning---selecting the action that minimizes total behavioral friction while satisfying all hard constraints---is $A^*$ search with the metric tensor providing edge weights and moral obligations serving as pre-compiled heuristic functions.
...
The four upgrades required for this transition are not speculative additions to the framework. Three of the four are already developed in the Geometric Ethics monograph \cite{bond2026ge}: the moral metric tensor (Chapters 6, 9), the moral Lagrangian (Chapter 11), and the Riemann curvature tensor with its holonomy interpretation (Chapters 8--10). The fourth---the moral field equation itself---is the principal open problem of the research program (\cite{bond2026ge}, \S19.4).
...
The moral manifold $\calM$ is a nine-dimensional Whitney-stratified space whose coordinate axes correspond to the fundamental dimensions of moral variation \cite{bond2026ge}:
...
The behavioral friction along a path $\gamma: [0,1] \to \calM$ is:
...
\begin{definition}[Moral Metric Tensor {\cite[Ch.~6]{bond2026ge}}]
A \emph{moral metric tensor} $g_{\mu\nu}$ on $\calM$ is a symmetric, positive-definite bilinear form assigning to each point $x \in \calM$ an inner product on the tangent space $T_x\calM$:
\[
g_{\mu\nu}(x): T_x\calM \times T_x\calM \to \R
\]
...
Empirically, the metric has been calibrated from a corpus of 20,030 Dear Abby letters, yielding dimension-specific weights that range from 0.72 (epistemic status) to 1.00 (physical harm) \cite{bond2026ge}.
...
The formal structure supporting this upgrade is already in place. The DEME 3.0 governance architecture specifies metric tensors as part of governance profiles \cite[Ch.~18]{bond2026ge}:
\begin{equation}
...
A feature of the dynamic metric that has no analogue in $A^*$ pathfinding: if the metric is \emph{degenerate} at a point---if $\det(g_{\mu\nu}(x)) = 0$---then some moral dimensions become literally incommensurable at that configuration. No trade-off ratio exists. This is the geometric formalization of value incommensurability, the phenomenon that moral philosophers have long recognized but lacked a mathematical framework to express.
...
The field-theoretic replacement is the \emph{moral Lagrangian} $\calL$ and the associated \emph{action functional} $\calA$.

\begin{definition}[Moral Action {\cite[\S11.4]{bond2026ge}}]
The moral action along a path $\gamma: [0,T] \to \calM$ is:
\begin{equation}
\calA[\gamma] = \int_0^T \calL(\gamma(t), \dot{\gamma}(t))\, dt
\end{equation}
where the moral Lagrangian is:
\begin{equation}\label{eq:lagrangian}
\calL(\gamma, \dot{\gamma}) = \frac{1}{2} g_{\mu\nu}(\gamma)\, \dot{\gamma}^\mu\, \dot{\gamma}^\nu - V(\gamma)
\end{equation}
\end{definition}
...
The Lagrangian formulation is not merely an elegant restatement of $A^*$. It enables \emph{Noether's theorem}, which connects symmetries of the Lagrangian to conserved quantities.

\begin{theorem}[Moral Noether's Theorem {\cite[Theorem 11.1]{bond2026ge}}]
If the moral Lagrangian $\calL$ is invariant under re-description $\gamma^\mu \to \gamma^\mu + \epsilon\, \xi^\mu$ (the Bond Invariance Principle), then the Noether charge
\begin{equation}
...
\begin{definition}[Moral Holonomy {\cite[\S8.9]{bond2026ge}}]
The holonomy group $\mathrm{Hol}(\nabla, p)$ at a point $p \in \calM$ is the group of linear transformations of the tangent space $T_p\calM$ generated by parallel transport around all closed loops based at $p$. If the holonomy is non-trivial, moral reasoning is \emph{path-dependent}: the order in which moral considerations are encountered changes the outcome.
\end{definition}
...
The curvature of moral space is the next empirical frontier. The monograph states this directly: ``Curvature should produce measurable holonomy (untested: the next empirical frontier)'' \cite[\S22]{bond2026ge}. The non-abelian property of the $D_4$ gauge group ($sr \neq rs$) provides indirect evidence: the order in which Hohfeldian state transitions are applied changes the outcome, which is a discrete analogue of holonomy. But direct measurement of the Riemann tensor components from empirical data remains open.
...
In general relativity, Einstein's field equation accomplishes this:
\begin{equation}
R_{\mu\nu} - \frac{1}{2}R\, g_{\mu\nu} = 8\pi G\, T_{\mu\nu}
\end{equation}
...
\begin{enumerate}
    ...
    \item \textbf{Unify dynamics and structure:} Connect the dynamics of moral evolution (Chapter 10) with the statics of moral space (Chapters 5--8).
\end{enumerate}

\subsection{What Progress Requires}

Three milestones define the research program \cite[\S19.4]{bond2026ge}:
...
The caller's framing references an ``Ethical Yang-Mills'' structure with gauge group $SU(2)_I \times U(1)_H$. This was indeed the original proposal. It has been \textbf{empirically falsified}.

\subsection{The CHSH Tests}

Direct tests of quantum contextuality in moral reasoning ($N = 600$ evaluations across four entangled-agent scenarios) found no violation of the CHSH inequality: all $|S| \leq 2$ \cite[\S12.9]{bond2026ge}. The Hardy non-locality test met 3 of 4 conditions but not the fourth. No-signaling conditions were satisfied in all scenarios ($p > 0.05$).
...
\begin{equation}
\boxed{\calG_{\text{ethics}} = D_4 \times U(1)_H}
\end{equation}
...
This is an example of the framework's empirical discipline: a beautiful theoretical prediction ($SU(2)$ quantum contextuality in ethics) was tested and falsified, and the theory was revised accordingly.
...
The monograph develops this relationship through the concept of \emph{compilation targets} \cite[\S20]{bond2026ge}:

\begin{quote}
``Each compilation target represents a different \emph{contraction} of the full ErisML specification---a mapping from the rich normative structure to a computationally tractable approximation. The information lost in compilation is the engineering analogue of the moral residue: normative structure that the computational backend cannot represent.''
\end{quote}
...
Each layer below is a \emph{contraction} of the layer above. Each contraction loses information---the moral residue $\calR$ of Chapter 14. The $A^*$ model is the computationally tractable layer; the field equation is the generating theory.

\subsection{The Churchill Connection}

The caller's framing invokes Churchill: $A^*$ is the ``best of the worst'' because it is what a human brain or a standard AI can actually compute. A full field theory requires solving 9-dimensional nonlinear partial differential equations in real time.

This is correct, and the framework is explicit about it. Stratified evaluation---the $A^*$ layer---is ``computationally tractable and produces intuitive decisions at boundary crossings'' \cite[\S9]{bond2026ge}. The field equation would be the theory that \emph{explains} why those decisions are intuitive, by deriving the metric from first principles rather than measuring it empirically.
...
\bibitem[Bond(2026a)]{bond2026ge}
Andrew~H. Bond.
\newblock \emph{Geometric Ethics: The Mathematical Structure of Moral Reasoning}, v1.15.
\newblock San Jos\'{e} State University, 2026.

\bibitem[Bond(2026b)]{bond2026pe}
Andrew~H. Bond.
\newblock Philosophy Engineering: A Foundation Document for the Discipline.
\newblock v1.0, San Jos\'{e} State University, 2026.
