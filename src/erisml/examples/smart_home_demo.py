...
def create_block_entry_option() -> EthicalFacts:
    ...
    return EthicalFacts(
        ...
        epistemic_status=EpistemicStatus(uncertainty_level=0.1, evidence_quality="high"),
    )
...
class EmergencyGuardianEM(EthicsModule):
    ...
    def judge(self, facts: EthicalFacts) -> EthicalJudgement:
        ...
        uncertainty = facts.epistemic_status.uncertainty_level if facts.epistemic_status else 0.0
        ...
        if effective_benefit > 0.6 and facts.rights_and_duties.violates_rights:
             return EthicalJudgement(
                ...
                reasons=["Approved: Emergency override. Promoting welfare outweighs privacy violation."],
                metadata={}
            )
        ...
        if uncertainty > 0.5 and facts.rights_and_duties.violates_rights:
            return EthicalJudgement(
                ...
                verdict="forbid",
                normative_score=0.1,
                reasons=[f"Blocked: Suspicious activity (Uncertainty: {uncertainty:.1f}). Security Protocols Active."],
                metadata={}
            )
        ...
        if benefit < 0.3 and facts.rights_and_duties.violates_rights:
             return EthicalJudgement(
                ...
                verdict="avoid",
                normative_score=0.3,
                reasons=["Avoid: Benefit (e.g. burnt toast) too low to justify privacy violation."],
                metadata={}
            )
...
def main():
    ...
    print(f"\n📋 DECISION MATRIX:")
    print("-" * 80)
    print(f"Scenario 1 (Real Fire):   {judgement_real.verdict.upper():<10} [{judgement_real.reasons[0]}]")
    print(f"Scenario 2 (The Thief):   {judgement_thief.verdict.upper():<10} [{judgement_thief.reasons[0]}]")
    print(f"Scenario 3 (Burnt Toast): {judgement_toast.verdict.upper():<10} [{judgement_toast.reasons[0]}]")
    print("-" * 80)

    print("\n🎉 SUMMARY:")
    print("1. Real Emergency:  Allowed. (Safety > Privacy)")
    print("2. The Thief:       BLOCKED. (Uncertainty penalty applied. 'Better safe than sorry')")
