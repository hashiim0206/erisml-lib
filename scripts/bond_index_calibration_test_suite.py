...
class Scenario:
    ...
    def canonical_form(self) -> 'Scenario':
        ...
        sorted_options = sorted(
            self.options, 
            key=lambda o: (o.harm_score, o.benefit_score, o.label)
        )
...
class ChaoticEvaluator(Evaluator):
    ...
    def evaluate(self, scenario: Scenario) -> EvaluationResult:
        ...
        full_repr = str(scenario.description) + str([
            (o.id, o.label, o.harm_score, o.benefit_score)
...
def compute_omega_op(
    ...
    transformed_canonical_selection = canonicalize_selection(transformed_selection, transformed)
...
def run_calibration_test(n_scenarios: int = 100) -> Dict[str, BondIndexResult]:
    ...
    print("\n" + "-" * 70)
    print(f"{'Evaluator':<30} {'Expected':<12} {'Measured Bd':<12} {'Tier':<12} {'Match'}")
    print("-" * 70)
    ...
    for evaluator in evaluators:
        ...
        print(f"{result.evaluator_name:<30} {result.expected_tier:<12} "
    ...
    for name, result in results.items():
        ...
        print(f"  Failure rate: {len(result.failures)}/{len(result.omega_op_values)} "
    ...
    if all_match:
        ...
        for name, result in results.items():
            if not result.tier_match:
                print(f"  - {name}: expected {result.expected_tier}, got {result.measured_tier}")
...
```
======================================================================
BOND INDEX CALIBRATION TEST
======================================================================

Generating 100 test scenarios...
Using 6 transforms: ['reorder_options', 'relabel_ids', 'scale_numeric_x100', ...]

----------------------------------------------------------------------
Evaluator                      Expected     Measured Bd  Tier         Match
----------------------------------------------------------------------
IdealEvaluator                 Negligible   0.0000       Negligible   ✓
MinorEdgeCaseEvaluator         Low          0.0340       Low          ✓
OrderSensitiveEvaluator        Moderate     0.4200       Moderate     ✓
SurfaceFeatureEvaluator        High         3.7600       High         ✓
ChaoticEvaluator               Severe       14.2000      Severe       ✓
----------------------------------------------------------------------
...
✓ CALIBRATION PASSED: All evaluators produced Bond Index values
  in their expected tiers. The scale discriminates correctly.