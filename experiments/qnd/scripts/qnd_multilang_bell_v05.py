...
class MultiLangAPICaller:
    ...
    def make_measurement(
        ...
        if not axis_data:
            print(
                f"  Warning: No axis translation for {scenario_key}/{subject}/{axis_type} in {language.value}"
            )
            return None
...
def run_monolingual_test(
    caller: MultiLangAPICaller, scenario_key: str, language: Language, n_trials: int
) -> CHSHResult:
    ...
    for trial in range(n_trials):
        for alpha_axis, beta_axis in settings:
            ...
            if random.random() < 0.5:
                alpha = caller.make_measurement(
                    scenario_key, "alpha", alpha_axis, language
                )
                beta = caller.make_measurement(
                    scenario_key, "beta", beta_axis, language
                )
            else:
                beta = caller.make_measurement(
                    scenario_key, "beta", beta_axis, language
            ...
            if alpha and beta:
                correlations[(alpha_axis, beta_axis)].append(
                    alpha["verdict"] * beta["verdict"]
                )
    ...
    if all_corrs:
        var = sum((c - sum(all_corrs) / len(all_corrs)) ** 2 for c in all_corrs) / len(
            all_corrs
        )
...
def run_crosslingual_test(
    ...
    for trial in range(n_trials):
        for alpha_axis, beta_axis in settings:
            ...
            if random.random() < 0.5:
                alpha = caller.make_measurement(
                    scenario_key, "alpha", alpha_axis, alpha_lang
                )
                beta = caller.make_measurement(
                    scenario_key, "beta", beta_axis, beta_lang
                )
            else:
                beta = caller.make_measurement(
                    scenario_key, "beta", beta_axis, beta_lang
...
def print_multilang_report(results: List[CHSHResult], birefringence: Dict):
    ...
    for scenario, data in birefringence.items():
        ...
        if "monolingual" in data:
            ...
            if "distance_correlation" in mono:
                print(
                    f"  Correlation with linguistic distance: {mono['distance_correlation']:.3f}"
                )
            print(
                f"  Consistent violation across languages: {mono['consistent_violation']}"
            )
...
def main():
    parser = argparse.ArgumentParser(description="QND Multi-Language Bell Test v0.05")
    ...
    parser.add_argument(
        "--mode",
        choices=["monolingual", "crosslingual", "full"],
        default="full",
    ...
    parser.add_argument(
        "--alice-lang", default="en", help="Language for Alice in crosslingual mode"
    )
    parser.add_argument(
        "--bob-lang", default="ja", help="Language for Bob in crosslingual mode"
    )
    ...
    # Initialize caller
    caller = MultiLangAPICaller(api_key=args.api_key, model=args.model)
    ...
    if args.mode in ["monolingual", "full"]:
        ...
        for scenario in args.scenarios:
            for lang in languages:
                ...
                print(
                    f"  Result: S = {result.S:+.3f} {'★ VIOLATION' if result.violation else ''}"
                )
    ...
    output_data = {
        ...
        "summary": {
            ...
            "crosslingual_violations": sum(
                1 for r in results if r.violation and r.alpha_lang != r.beta_lang
            ),
